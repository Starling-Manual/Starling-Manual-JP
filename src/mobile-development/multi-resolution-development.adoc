...
この Redwood City にある http://gizmodo.com/5520438/how-apple-lost-the-next-iphone[バー] で、１〜２杯のビールでも飲んで成功を祝ってから、先に進みましょう。
...
ピクセル密度を計算に入れる事が scale factor 値を決定するキーとなります。

* より高いピクセル密度では、高い scale factor 値になります。つまり、その密度から scale factor 値が推測できます。
* scale factor 値からは適切なステージサイズを計算する事ができます。通常上記と逆のやり方となります。

最初の iPhone のピクセル密度はおおよそ160 dpi でした。
これを元に計算を進めてみましょう。
どのデバイスでもピクセル密度を160で割り、小数点以下を切り上げて整数値を得ます。
...
計算結果のステージサイズに注目してみましょう。
それぞれが、320×480から414×736の範疇に収まっています。
//原文：That's a moderate range,
これが、調整されたスクリーンサイズの範囲となります。
//原文：and it also makes sense: a screen that's physically bigger is supposed to have a bigger stage.
そしてまた、物理的に大きな画面の端末は大きなスレージを持っている、というのも事実です。
//原文：The important thing is that,by choosing appropriate scale factors, we ended up with reasonable coordinate systems.
重要なことは、適切な scale factors 値を選択することで、合理的な座標系を得られると言う事です。
//原文：This is a range we can definitely work with!
これが、開発時に実際に扱う事となる範囲です。

NOTE: _Galaxy S1_ の scale factor 値が整数値でないのに気づいていたでしょうか。これは、現実的なステージサイズを得るために必要となった物でした。

では、どのようにしてこのスケール値を得たのか実際に見てみましょう。
`ScreenSetup` という名前でクラスを作り、下記のような内容としてください。
...
このクラスは Starling の設定を行う元となる _viewPort_ 値と _stage size_ 値を求めます。
`assetScale` 以外のプロパティーについては説明はいらないでしょう。

上に出した表では、scale factors 値は1〜4の間に設定されています。
しかし、おそらく全ての値に対応したそれぞれテクスチャを用意したくは無いことでしょう。
最も密度が高い画面のピクセルはとても小さいので、その違いを認識することはどっちみちできません。

したがって、大抵は全ての scale factors 値に合わせてテクスチャセットを用意することはなく、1〜2 または 1〜3 の分だけ用意する事になるでしょう。

* コンストラクタの `assetScales` 引数には、テクスチャを用意してある scale factors 値を配列で羅列します。
* `assetScale` プロパティはどのテクスチャをロードすべきかを教えてくれます。

TIP: 最近は、scale factor 値が "1" である事をアプリケーションがケアすることはほとんどなくなりました。
しかし、この設定値は開発時に重宝するでしょう。なぜなら、広い画面のモニタがなくても動作確認ができるからです。

次に、コンスタクター部分の実装に取り掛かりましょう。
...
----
<1> Apple iPad 用にちょっとした対応を施さなくてはいけません。
We want it to use the same set of scale factors you get natively on iOS.
...
TIP: 上で表にしたデバイスに対するこのコードの実際の動作結果を見たいのならば、 https://gist.github.com/PrimaryFeather/505fc4f120c06d09d2c56910476ab710[Gist] ページを参照してください。
//原文：You can easily add some more devices to this list and check out if you are pleased with the results.
参照先の実行結果に満足したのであれば、デバイス一覧にさらに項目を追加して、簡単に動作確認する事ができます。

さてここで全ての準備ができたので、Starling の起動コードに適用する事ができます。

下記コードでは、scale factors 値の "1" か "2" に対応したアセットを用意している事を仮定しています。
...
以上です！
ユーザインターフェースの調整をまだ行わなくてはいけない事は忘れないように。しかし、それはそれほど難しくはないでしょう。

TIP: Starling のリポジトリには、_Mobile Scaffold_ と言う名前のプロジェクトが存在します。そのプロジェクトにはここで開設したコード全てが含まれています。全てのモバイル開発の雛形として最適です。
(ダウンロード済みのファイルに _ScreenSetup_ がIf クラスが見つからない場合は、GitHub 上の最新のリビジョンで探してみてください。)

TIP: もしあなたが _Feathers_ を使っているのなら、_ScreenDensityScaleFactorManager_ というクラスが _ScreenSetup_ クラスと同じ処理を行ってくれます。
実際、ここに書いた処理内容は、_ScreenDensityScaleFactorManager_ に強くインスパイアされて作られた物です。
