...
これで実際に _NGon_ クラスに必要な全ては完了です！
あとは上記メソッドをコンストラクタ内で使用してやります。

その他、ディスプレイオブジェクトとして必要な処理（あたり判定、描画処理、バウンズの計算、など）は親クラスが処理してくれます。
...
この手法は他のどんな形状のシェイプにも適用できます。

[TIP]
====
カスタムメッシュを扱う際は、`starling.geom` パッケージに存在する _Polygon_ クラスも参考にしてみてください。

このクラスは、いくつもの頂点で定義される任意の閉じた形状のシェイプを三角形に分割する際に役立ちます。
詳細は <<Masks>> セクションでさらに確認します。
====

=== テクスチャの追加

このポリゴンにテクスチャを適用できたら良いと思いませんか？
ベースクラスの _Mesh_ は、すでに `texture` プロパティを持っています。 _テクスチャの座標設定_ をまだ行なっていないだけです。

テクスチャの座標設定を行うことによって、テクスチャのどの部分を頂点に適用するか決定する事になります。
//原文：which is a reference to the names that are typically used for their coordinate axes (_u_ and _v_).
//うまく訳せない。。
一般的に、_u値、v値_ で表される座標軸で場所を示すので、しばしば _UV座標_ と呼ばれます。
...
この情報を元に、`createVertexData` メソッドを適切に調整する事ができます。
...
テクスチャが割り当てられると、レンダリングを担当する処理が自動的にそれらの設定を取得します。
...
NGonのエッジをとく観察してみると、ジャギーが発生している事に気づくかもしれません。
これは、GPU がピクセルを n-gon の内側か外側かでしか判定せず、中間の状態がないからです。
これを修正するために、アンチエイリアス処理を有効にする事ができます。_Starling_ クラスのプロパティにそれがそのままの名前で存在します。
...
この値は _Stage3D_ がレンダリングする際使うサブサンプリング数に関係します。
...
たくさんのサブサンプリングを行うためには、たくさんの計算が発生します。アンチエイリアス処理は高コストな処理なのです。
さらに言えば、_Stage3D_ は全てのプラットフォームでアンチエイリアス処理をサポートしていません。

NOTE: モバイル環境では、アンチエイリアス処理は _RenderTextures_ 内だけで利用が可能です。

よって、これは理想的な解決方法ではありません。
//原文：The only consolation I can offer:
唯一の救いとなる情報としては、
一般的に画面のピクセル密度はどんどんと上昇しているという事です。

近年のハイエンドなモバイルフォンではピクセルの大きさがとても小さいため、もはやジャギーはあまり問題とはならないのです。
...
そこでは Starling に搭載されたスタンダードなレンダリングの方法を用いています。

さて、では、レンダリングの方法をのものをカスタマイズしたい場合はどうしたら良いでしょう？
...
_Mesh_ クラスのメソッドとプロパティは手堅い基礎機能を提供してくれますが、遅かれ早かれ、その機能だけでは物足りなくなってくる事でしょう。
//原文：Coming to the rescue: Starling's _mesh styles_.
Starling の _メッシュスタイル_ がこれを手助けします。
...
`スタイル` は Starling2.0 で導入された新機能で、高速に描画が可能なカスタムレンダリングコードを作成するための推奨される方法です。
//原文：In fact, all rendering in Starling is now done via mesh styles.
実際、Starling 内の全ての描画処理は、メッシュスタイルを通して実行されています。

* どのメッシュにもスタイルを１つ設定する事ができます。 (_Mesh_ のインスタンスもしくは、_Mesh_のサブクラスのインスタンスに対して。).
* デフォルトでは、それぞれのメッシュに紐づいているのは、基底 _MeshStyle_ クラスのインスタンスです。
* 後者は Starling の標準的な描画能力を提供します。色味がついてテクスチャが設定された三角形の描画です。

//原文：To teach your meshes new tricks, you can extend _MeshStyle_.
メッシュに新しい見た目を加えるには、_MeshStyle_ を継承すると良いです。
//原文：This allows you to create custom shader programs for all kinds of interesting effects.
カスタムのシェーダープログラムを作って、様々な種類のエフェクトを実現することができます。
例えば、複数のテクスチャの割り当てや、カラーの高速トランジション効果などが実装可能です。

[TIP]
====
最も印象的なスタイルのサンプルとては、 http://wiki.starling-framework.org/extensions/dynamic_lighting[Dynamic Lighting extension] があります。
ノーマルマップ（ポリゴン面の法線方向をエンコードしたテクスチャ）を用いることで、リアルな照明効果を動的に適用することができます。
the Starling Wiki を確認して、実際にこの拡張機能が動いている例を見て見てください。
====

スタイルを使うには、まずインスタンス化をし、メッシュの `style` プロパティとして設定します。
...
.動的な照明処理の拡張機能が動いている様子。
image::dynamic-lighting.png[Dynamic Lighting]

スタイルでできることは本当にたくさんあり、適用範囲にはほとんど限界がありません。

//原文：And since meshes with the same style can be batched together, you do not sacrifice performance in any way.
そして同じスタイル設定のメッシュはバッチ処理で同時に扱われるので、パドーマンスを気にする必要もありません。
この点に関して、スタイルは同じような目的も用いられるフラグメントフィルターよりもずっと効果的です。

スタイルの主な短所はそれがメッシュだけに適用でき、スプライトには適用できないという点です。
そして、スタイルはメッシュの描画領域の範囲内のみでしか有効となりません。ぼかし効果のような事はできないのです。
さらに、一つのメッシュに対して複数のスタイルを組み合わせて使う事はできません。

それでも：スタイルは どんな Starling 開発者でも身につけて使うべきパワフルなツールです。
...
続く <<Custom Styles>> のセクションも続いて確認してください。
スクラッチでカスタムスタイルを作成する方法やその他シェーダーなどについて解説します。

[NOTE]
====
もしまだ、 _メッシュ_ と _メッシュスタイル_ の違いについてよく理解できていないのであれば、このように考えてみてください。
_メッシュ_ は頂点の集合でしかなく、どのように頂点で三角形を描くかを示しているだけであると。

スタイルは頂点に追加の情報を設定し、レンダリング時に使用します。
標準の _MeshStyle_ はカラーとテクスチャの割り当てを提供します。
_MultiTextureStyle_ は追加のテクスチャの割り当て情報などを設定するのかもしれません。
しかし、スタイルは決して元々のオブジェクトの形状を変更しません。
