...
トゥイーンが終了した際、ジャグラーからは自動的に登録が取り除かれます。
もしもアニメーションを途中で中断したい場合は、ジャグラーからシンプルに取り除いてください。
...
技術的な面では、Starling がサポートするすべてのアニメーションをカバーし終えました。
しかし、このトピックに深く関係する、また別のコンセプトを持つ機能があります。
...
代わりに、そうですね、２秒ほど後に実行してください。（プレイヤーが自体が変化した事を把握するのに十分な時間です。）
//原文：(time enough for the player to realize the drama that is unfolding).

この遅延を実装するには、ネイティブの _Timer_ か `setTimeout` メソッドを使うことができます。
しかし、ここでジャグラーを利用する事もできます。そしてその方が、全てを制御する事ができるという大きな利点があります。

プレイヤーがその２秒が経たたないうちにポーズボタンを押してしまような場合を考えると、その利点が明らかになるでしょう。
その場合は、ゲームエリアのアニメーションを一時的に止めるだけでなく、この `gameOver` 呼び出しのタイミングも一時停止させておきたいはずです。

そのためには、下記のように処理を実行しましょう。
...
このメソッドには引数を渡す事も可能です。代わりにイベントを実行したいでしょうか。
...
delayed コールを使う別の便利な方法は、繰り返しの処理実行を行う事です。
...
遅れて処理が実行されるのを避けるには、下記のうちどちらかのメソッドを実行してください。
