...
経路上であればどこでも、このイベントを購読できます。犬の親の階層でもステージでもそれが可能です。
...
上記のサンプルではシングルタッチのみを購読していました。（つまり、指一本だけ。）
それがマルチタッチでもほとんど同じように取り扱う事ができます。たった一つの違いは、代わりに `touchEvent.getTouches` 命令を呼び出すと言う事だけです。（"Touch" が複数形になっています。）
...
`getTouches` メソッドは、タッチのベクター型配列を返します。
そのベクターの長さと中身を元にロジックを組み立てる事が出来るのです。

* 最初のif節では、指一本だけがスクリーンに触れています。`getMovement` メソッドを通して、例えばドラッグジェスチャーのような物を実装可能です。
* else節では、２本(以上)の指がスクリーンに触れています。２つのタッチオブジェクトにアクセスして、ピンチジェスチャーのような物を実装可能です。

NOTE: このデモアプリは Starling のダウンロード内容の一部として含まれています。 https://github.com/Gamua/Starling-Framework/blob/master/samples/demo/src/utils/TouchSheet.as[TouchSheet] クラスが _マルチタッチ_ シーンで利用されています。
タッチハンドラによってスプライトをドラッグ、回転、スケーリングする事が出来るサンプル実装となっています。

==== マウスアウトとエンドホバー

マウスが（マウスボタンが押されていない状態で）オブジェクトから離れる時に関して存在する特殊なケースについて考えます。
(これはタッチではなく、マウス入力だけに関連した考察です。)

ホバーしているオブジェクトが別のものに変化したのなら、_TouchEvent_ が前のホバー対象に対して発行され、もうすでにホバーされていない事を通知します。
そして、この際には、`getTouch` メソッドが null を返します。
どう言うものがマウスアウトイベントと呼ばれるの理解できたでしょうか。
