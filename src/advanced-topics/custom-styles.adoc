...
さて、ここでこういう疑問が湧くかもしれません。同士てこんな作業が必要なのか？フィルターでは頂点フォーマットを調整しなくてもうまくいったじゃないか、と。

これはとても良い質問です。質問されて嬉しいですね！
答えは、Starlng のバッチ処理コードにあります。
同じスタイルを次に描画するメッシュに適用する際、それらは一緒にバッチ処理されます。これが上記のような労力を払う理由です。

でも、バッチ処理とはそもそもなんでしょう？
それは、個々のメッシュの頂点データをまとめて１つの大きなメッシュとしてレンダリングする事を意味します。
Starling のレンダリング処理の中で、下記のようなコードを見つけることができるでしょう。
...
ここで問題となる箇所がわかるでしょうか？大きなメッシュ（＝`batch`）は最初のメッシュのスタイルのコピーを適用されています。
しかし、３つのメッシュのスタイルはそれぞれ異なった設定になっていたかもしれません。

もしもそれらの設定が _style_ の中にだけ保持されているとしたら、最初の１つをメッシュをのぞいてその設定は失われてしまいます。
そうはならいように、スタイルはデーターをターゲットメッシュの頂点に格納しなくてはいけません。
その場合だけ、巨大なメッシュは全てのオフセット値を個別に受け取ることができます。

IMPORTANT: この事はとても重要なので、繰り返してお伝えします。
*スタイルの設定はいつでもターゲットとなるメッシュの頂点データに格納される必要があります。*

頂点フォーマットはいつでもスタイルクラスの静的定数としてアクセスする事ができ、`vertexFormat` プロパティとしてアクセスが可能です。
...
スタイルがメッシュに適用された際、メッシュの頂点は自動的に新しいデータフォーマットに適合されます。

このコンセプトを理解したのなら、このチャプターの半分はやり遂げたことになります。
残りは、フラグメントシェーダーの定数ではなく、頂点データからオフセット値を読み出すようにコードをアップデートするだけです。
...
スタイルクラスの設定は終わりました。次は実際のレンダリングが行われているエフェクト側へ移りましょう。
今度は _MeshEffect_ クラスを継承します。
エフェクトはローレベルなレンダリングコードを書くだけである事を思い出してください。
下記のような継承関係クラスでグループ分けして解説を進めています。
...
ベースクラス（ _Effect_ ）は、とても小さいクラスです。三角ポリゴンを描画するだけとなっています。
_FilterEffect_ は、それにテクスチャのサポートを加え、_MeshEffect_ はそこにカラーとアルファの扱いを加えます。

NOTE: これら２つのクラスは _TexturedEffect_ と _ColoredTexturedEffect_ という名前にする事もできました。しかし、実際にそのクラスが利用される場所を踏まえて別の名前としました。
フィルターを作るのであれば、_FilterEffect_ を継承する必要があります。スタイルを作るのであれば、_MeshEffect_ を継承します。
...
１つ前のチュートリアルのアナログフィルターエフェクトと比較すると、全てのオフセット値設定が削除されているのがわかるでしょう。
その代わりに `vertexFormat` メソッドをオーバーライドしています。オフセット値を頂点データとして格納するため、スタイルに対応した頂点フォーマットを使っています。
